...
import { DashboardRunRepository, type HistoricalRunSummary } from "../ui/dashboard-run-repository.js";
...
export async function handleResumeCommand(
  ...
  const eligible = summaries.filter(
    s => (s.status === "paused" || s.status === "failed" || s.status === "cancelled") && s.latestCheckpoint && !s.resumeBlockedReason
  );
  ...
  const paused = eligible.filter(s => s.status === "paused");
  const terminal = eligible.filter(s => s.status !== "paused");
  const sorted = [...paused, ...terminal].slice(0, MAX_RESUME_CANDIDATES);

  const labels = sorted.map(s => {
    const checkpoint = s.latestCheckpoint!.cursor;
    const request = truncate(s.request, 50);
    const decision = s.status === "paused" && s.waitingFor ? ` | ${truncate(s.waitingFor, 36)}` : "";
    return `${s.id} | ${s.status === "paused" ? "paused" : s.status}${decision} | ${checkpoint} | ${request}`;
  });
